...
import { InternalPairKeysListMap } from "../../structs/map.js";
/** Cache "Crossfade Key" -> "enter / leave" -> Element. */
const CrossFadeElementMatchMap = /*#__PURE__*/ new InternalPairKeysListMap();
/**
 ...
 */
export function setCrossFadeElementForPairOnly(key, el) {
...
export function deleteCrossFadeElementForPairOnly(key, el) {
    CrossFadeElementMatchMap.delete(key, 'any', el);
}
/** Get element for crossfade transition pair. */
export function getCrossFadeElementForPhase(key, phase) {
 ...
 */
export const crossfade = /*#__PURE__*/ Transition.define(async function (el, options, phase) {
    CrossFadeElementMatchMap.add(options.key, phase, el);
    // Sync same keyed enter and leave transitions.
    await UpdateQueue.untilAllComplete();
    let pairPhase = phase === 'enter' ? 'leave' : 'enter';
 ...
 */
function fromBoxPair(fromBox, toBox, fitMode = 'stretch') {
