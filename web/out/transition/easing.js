...
const EasingFunctionCache = { ...CustomEasingFunctions };
...
export function getEasingFunction(name) {
    if (EasingFunctionCache[name]) {
        ...
        let [x1, y1, x2, y2] = CubicBezierEasingParameters[name];
...
function makeCubicBezierEasingFunction(x1, y1, x2, y2) {
    ...
    return function (x) {
        ...
        return toDecimal(((ay * t + by) * t + cy) * t, 5);
    };
}
/**
...
export function getCSSEasingValue(easing) {
    return CubicBezierEasingParameters.hasOwnProperty(easing)
        ? 'cubic-bezier(' + CubicBezierEasingParameters[easing].join(', ') + ')'
        : easing;
}
